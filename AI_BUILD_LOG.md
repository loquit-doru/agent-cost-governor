...
### Phase 7: AI Decision Zone + Smart Pattern Matching (Feb 17, 2026)
**Human-driven decisions:**
- Decided AI should DECIDE (not just explain) in the gray zone
- 3-zone model design: safe (≤5) → gray (6-10, AI decides) → storm (>10, hard block)
- Self-dogfooding strategy: integrate ProceedGate into TokenSentry (apix402)
- Chose behavioral signals: interval CV, backoff detection, cost accumulation, similarity grouping

**AI-assisted:**
- `aiDecideGrayZone()` — real LLM decision function (Llama 3.1 8B, 0.1 temperature)
- `DECISION_SYSTEM_PROMPT` — structured prompt for allow/block decisions with 7 behavioral signals
- `heuristicGrayZone()` — fallback scoring system (suspicion ≥5 = block)
- `detectBackoff()` — identifies exponential backoff (intervals growing = responsible agent)
- `trackSimilarityGroup()` — groups parameter variants under shared prefix
- Cost accumulation tracking in Durable Object (`cost_window_usd`)
- ProceedGate middleware for apix402 (`proceedgate.ts`) — fail-open, 400ms timeout
- CORS header exposure for governance signals
- End-to-end testing of gray zone AI decisions (live deployment)
