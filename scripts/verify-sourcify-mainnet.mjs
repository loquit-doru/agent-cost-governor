...
const CONTRACT_ADDRESS = '0x161D749892a23AC8792eE7fD37f0F423E0b69C97';
...
function compileWithMetadata() {
  const source = readFileSync(resolve('contracts', 'AICostGovernor.sol'), 'utf-8');
  const input = {
    language: 'Solidity',
  ...
  return output.contracts['AICostGovernor.sol'].AICostGovernor.metadata;
}

async function main() {
  console.log('Compiling with metadata...');
  const metadata = compileWithMetadata();
  console.log('Compile OK');

  // Build multipart form for Sourcify
  ...
  if (res.ok && (json.result?.[0]?.status === 'perfect' || json.result?.[0]?.status === 'partial')) {
    ...
    console.log('BscScan Sourcify tab:', `https://bscscan.com/address/${CONTRACT_ADDRESS}#code`);
  } else {
    console.log('\nSourcify response:', JSON.stringify(json, null, 2));
